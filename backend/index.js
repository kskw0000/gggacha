      ...
      winProbability REAL
        ...
        (err) => {
          if (err) {
            ...
            db.get(
              `SELECT COUNT(id) as count FROM gacha_settings`,
              (err, row) => {
                if (err) {
                  console.error(err.message);
                } else if (row.count === 0) {
                  db.run(
                    `INSERT INTO gacha_settings (id, wins, rolls, winProbability) VALUES (1, 0, 0, 0.0)`
                  );
                }
              }
            );
...
function writeToDb(winCode, userId) {
  ...
  db.run(
    `INSERT INTO win_codes (winCode, issuedAt, userId) VALUES (?, ?, ?)`,
    [winCode, issuedAt, userId],
    (err) => {
      if (err) {
        console.error(err.message);
      }
...
app.get('/auth/line', async (req, res) => {
  ...
  try {
    ...
    // 保存: ユーザーIDとアクセストークン
    db.run(
      `INSERT OR REPLACE INTO user_tokens (userId, accessToken) VALUES (?, ?)`,
      [userId, accessToken],
      (err) => {
        if (err) {
          console.error(err.message);
...
app.get('/gacha', async (req, res) => {
  ...
  db.get(
    `SELECT wins, rolls, winProbability FROM gacha_settings WHERE id = 1`,
    async (err, row) => {
      if (err) {
        console.error(err.message);
        ...
        db.run(
          `UPDATE gacha_settings SET wins = ?, rolls = ? WHERE id = 1`,
          [row.wins, row.rolls],
          (err) => {
            if (err) {
              console.error(err.message);
              ...
              res.json({
                isWin,
                winCode,
                availableWins: row.wins,
                availableRolls: row.rolls,
              });
            }
          }
        );
...
app.post('/admin/update-gacha', async (req, res) => {
  ...
  db.run(
    `UPDATE gacha_settings SET wins = ?, rolls = ?, winProbability = ? WHERE id = 1`,
    [wins, rolls, winProbability],
    (err) => {
      if (err) {
        console.error(err.message);
        res.status(500).send('An error occurred.');
