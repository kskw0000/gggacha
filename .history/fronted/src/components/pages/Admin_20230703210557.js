...
const Admin = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.post(`${process.env.REACT_APP_SERVER_URL}/admin/update-gacha`, { wins, rolls, winProbability: winProb });
  ...
  const getGachaInfo = async () => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_SERVER_URL}/admin/gacha-info`);
