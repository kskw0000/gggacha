...
app.get('/gacha', async (req, res) => {
  ...
  const winProbability = Number(settingsRow.winProbability) / 100; // Assuming this is stored as a percentage
...
app.get('/gacha/info', async (req, res) => {
  try {
    ...
    const rows = await sheet.getRows();
    const settingsRow = rows[0];
    res.json({ availableWins: settingsRow.availableWins, availableRolls: settingsRow.availableRolls, winProbability: settingsRow.winProbability });
  } catch (error) {
...
app.post('/admin/update-gacha', async (req, res) => {
  ...
  settingsRow.winProbability = winProbability;
  await settingsRow.save();
