    ...
    if (err) {
      ...
      db.query(`SELECT COUNT(id) as count FROM gacha_settings`, (err, results) => {
        if (err) {
          ...
          db.query(`INSERT INTO gacha_settings (id, wins, rolls, winProbability) VALUES (1, 0, 0, 0.0)`, (err) => {
            if (err) {
              console.error(err.sqlMessage);
            }
          });
...
app.get('/gacha', async (req, res) => {
  db.query(`SELECT wins, rolls, winProbability FROM gacha_settings WHERE id = 1`, async (err, results) => {
    try {
      if (err) {
...
app.post('/admin/update-gacha', async (req, res) => {
  ...
  db.query(`UPDATE gacha_settings SET wins = ?, rolls = ?, winProbability = ? WHERE id = 1`, [wins, rolls, winProbability], async (err) => {
    try {
      if (err) {
        throw new Error(err.sqlMessage);
      } else {
